...
router.post("/admin/partners", requireAdmin, async (req, res) => {
  const { name, cnpj, address, phone, email, masterName, masterCpf, masterEmail } = req.body as Record<string, string | undefined>;
...
router.put("/admin/partners/:id", requireAdmin, async (req, res) => {
  ...
  try {
    const [partner] = await db.update(partnersTable).set(updates as Parameters<typeof db.update>[0]).where(eq(partnersTable.id, id)).returning();
