...
export function RouteMap({ routes, employees = [], companyLat, companyLng, maxRadiusKm = 1, vehicleLabel }: RouteMapProps) {
  ...
  const blocks = useMemo(
    () =>
      [...new Set(routes.map((r) => r.vehicleBlockId).filter((v) => v != null) as number[])].sort(
        (a, b) => a - b
      ),
    [routes]
  );
  ...
  const bpColorMap = useMemo(() => {
    ...
    for (const r of routes) {
      const color = blockColor(r.vehicleBlockId ?? 1);
  ...
  useEffect(() => {
    ...
    Promise.all(
      toFetch.map(async (route) => {
        ...
        const sorted = [...route.boardingPoints].sort(
      ...
      <div className="w-52 shrink-0 bg-white dark:bg-card border rounded-xl p-4 flex flex-col gap-4 overflow-y-auto shadow-sm">
        ...
        <div className="border-t pt-3">
          ...
          <div className="space-y-1">
            {blocks.map((blockId) => {
              ...
              const checked = selectedBlocks.has(blockId);
              return (
                <label
                  key={blockId}
        ...
        <div className="mt-auto border-t pt-3 space-y-1.5 text-xs text-muted-foreground">
          <div className="flex items-center gap-2">
            <span className="w-4 h-4 rounded-full bg-green-600 border-2 border-white shadow-sm shrink-0" />
            Empresa (destino)
          </div>
          <div className="flex items-center gap-2">
          ...
          {showEmployees && (
            <div className="flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-gray-700 border border-white shadow-sm shrink-0" />
              Funcionário
            </div>
          )}
          {showCircles && (
            <div className="flex items-center gap-2">
              <span className="w-4 h-2 rounded-sm border-2 border-blue-400 bg-blue-100/60 shrink-0" />
              Raio {maxRadiusKm} km
            </div>
          )}
          {loadingOsrm && (
            <p className="text-amber-600 font-medium mt-2">Carregando rotas por vias…</p>
          )}
        </div>
      </div>
      ...
      <div className="flex-1 rounded-xl overflow-hidden border shadow-sm">
        ...
        >
          ...
          <Marker position={center} icon={companyIcon}>
            <Popup>
              <strong>Empresa (Destino)</strong>
            </Popup>
          </Marker>

          {/* Rotas */}
          {visibleRoutes.map((route) => {
            const color = blockColor(route.vehicleBlockId ?? 1);
            const sorted = [...route.boardingPoints].sort(
              (a, b) => (a.sequenceOrder ?? 0) - (b.sequenceOrder ?? 0)
            );
            ...
            return (
              <Fragment key={route.id}>
                <Polyline
                  ...
                  pathOptions={{
                    color,
                    weight: roadCoords ? 4 : 2,
                    opacity: 0.85,
                    dashArray: roadCoords ? undefined : "6,5",
                  }}
                />
                      ...
                      <Popup>
                        <div className="text-sm space-y-1 min-w-[160px]">
                          ...
                          <p className="text-muted-foreground">
                            {bp.passengerCount} passageiro{bp.passengerCount !== 1 ? "s" : ""}
                          </p>
          ...
          {/* Funcionários geoespacializados */}
          {showEmployees &&
            employees
                ...
                <Marker key={emp.id} position={[emp.lat, emp.lng]} icon={empDotIcon}>
                  <Popup>
                    <div className="text-xs space-y-0.5 min-w-[180px]">
                      ...
                      {emp.boardingPointId && (
                        <p style={{ color: bpColorMap.get(emp.boardingPointId) ?? "#555" }}>
                          ● Ponto {emp.boardingPointId}
                        </p>
                      )}
                    </div>
                  </Popup>
